...
| `Magyar Szlengszótár.pdf` / `Magyar-Szlengsz-t-r.pdf` (mangled-encoding duplicate filename, same file) | 170 | No | **High priority despite needing vision-reading** — **in progress, PDF pp. 1–90 of 170 sampled** | Hungarian slang dictionary — genuine slang content sitting in `source_reference/`, not a grammar reference. Belongs conceptually to `language_corpus/Hungarian/` once extracted (see scope note below), extracted from here since that's where it physically lives. PDF pp. 1–45 and 46–90 sampled (not exhaustive) into `language_corpus/Hungarian/02_szlengszotar_kovecses_A-E_sample.md` and `01_szlengszotar_dictionary_pp90-179.md` respectively (letters A through "link"); PDF pp. 91–170 (~letters m–z) still unread. |
