# {{Language}} — Established Vocabulary/Grammar: {{Lesson/Chapter/Section Label}}

**Source:** {{Author(s), Title (edition, year), specific section/pages}}
...
this, only ones where an atomic gloss would hide real compositional structure — add a breakdown
directly under the table row it belongs to. See
`../../00_Word_Concept_and_Morphological_Typology_Guide.md` for the worked Finnish example
(`pikkupukkikaupungissamme` = `pikku` "small" + `pukki` "goat" + `kaupungi-` "town" + `-ssa`
"in" (inessive) + `-mme` "our"). Format:

> **{{term}}** = {{morpheme1}} ({{gloss1}}) + {{morpheme2}} ({{gloss2}}) + ... — {{brief note on
> what the composition demonstrates, if relevant}}

## Grammar points

*(One subsection per grammar point covered in this section — name the rule, paraphrase the
explanation, cite the page. Direct quotes should be short and illustrative, not verbatim
reproduction of the source's own explanatory prose.)*

### {{Grammar point name}}

{{Paraphrased explanation, with a short illustrative example if useful.}}
