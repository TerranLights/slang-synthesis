# Serbian/Croatian/Bosnian — Established Vocabulary/Grammar: Magner, Lesson 1 (vision-reading test)

**Source:** Thomas F. Magner, *Introduction to the Croatian and Serbian Language* (Revised Edition,
Penn State University Press, 1991; 1998 update), "Prva lekcija" / "First Lesson," printed pp. 1-6
(plus the tail of the preface, printed p. xii, which fell on the same scanned image as the start of
Lesson 1). PDF pages 7-12 of `12.Introduction to the Croatian and Serbian language.pdf`.

**Status: vision-reading reliability test.** This shard was produced by reading a scanned,
image-only PDF (no text layer) directly via the Read tool's vision rendering, per an explicit test
dispatch. See the "Vision-reading confidence notes" section at the end for an honest self-assessment
...
`(S.)` regional-variant tag the source itself marks. A handful of items from the tail of the front-
matter preface (printed p. xii) are also included, flagged separately below, since that page shared
a scanned image with the start of Lesson 1 and contains genuinely useful comparative-phonology
content (false friends, accentuation). Repeated drill exercises (`vježba/vežba` I-IV sentence lists)
are skipped except where they introduce new vocabulary not already captured elsewhere (the "Nouns"
box, and the `slijèdeća/slédeća` variant pair). Lesson 2 content ("Druga lekcija"), which begins
partway through the same PDF page range, is explicitly **out of scope** for this dispatch and was
not extracted.
...
gender triplets like `dòbar, dòbro, dòbra` are listed as a single row with all three forms, matching
the source's own presentation, rather than decomposed as morphemes.)*

## Grammar points

### Ijekavian (Croatian) vs. ekavian (Serbian) jat reflex

The book's core comparative axis: wherever Common Slavic jat historically stood, standard Croatian
reflects it as `ije` (long jat) or `je` (short jat), while standard Serbian reflects it as `e` in
both cases. Long-jat examples from Lesson 1: `lijèp/lép` ("beautiful"), `slijèdeća/slèdeća`
("following"). Short-jat examples: `djèvojka/dèvojka` ("girl"), `vjèžba/vèžba` ("exercise"),
`rjèčnik/rèčnik` ("dictionary"), `čòvjek/čòvek` ("man, person"). p. 2-4.

### Emphatic vs. unemphatic copula

`je` ("is") and `sam`/`ste` ("am"/"are") are unemphatic, unstressed forms used in ordinary
statements. `jèst` (Cr.)/`jèste` (S.) is the emphatic counterpart of `je`, used for contrast or as
a standalone affirmative answer — the dialogue answers "Jè li ùdata?" with a bare "Jèst."/"Jèste."
meaning "Yes, [she is]." p. 2-3.

### tkò/kò ("who")

Croatian `tkò` vs. Serbian `kò` for the interrogative "who," tagged explicitly in the vocabulary
list and used in the dialogue's "Tkò/Kò je tá djévojka/dévojka, Péro?" p. 2-3.

### Yes/no questions with *li*

`li` marks a yes/no question, but its placement differs by standard in a way that echoes the
pattern already documented from the Alexander & Elias-Bursać textbook (`established/
001_lesson01_basics.md`): the Croatian dialogue line asks "Jè li ùdata?" (li directly after the
copula), while the Serbian line asks "Dà li je ùdata?" (`da li` fronted, copula after). Two
independent sources now attest the same broad Cr./S. word-order preference for this construction.
p. 1-3.

### Personal pronoun use for emphasis or disambiguation

Pronouns are normally dropped because verb morphology (person/number endings, or in the copula's
case, the unemphatic forms `sam`/`si`/`je`/`smo`/`ste`/`su`) already carries the necessary
information — hence `gòvorim` alone means "I speak/am speaking." An explicit pronoun is added either
for emphasis (`já gòvorim`, "I am the one speaking") or to disambiguate the third person, where
`Kàko se zòve?` is preferred over the ambiguous-by-gender `Kàko se ón/òna zòve?` unless the speaker
specifically wants to stress "he" or "she." p. 5.

### Adjective gender agreement

Adjectives inflect for the noun's grammatical gender: masculine, neuter, feminine. `dòbar` (masc.),
`dòbro` (neut.), `dòbra` (fem.) is the model triplet given for Lesson 1. The source's own footnote
explains why it lists forms in masc./neut./fem. order specifically: for most other adjectives the
neuter and masculine forms are identical, so giving the neuter right after the masculine makes any
exception (like `dobar`/`dobro` here, which do differ) easy to spot. p. 2.

### No vowel reduction

Unlike Russian (and unlike English's schwa-reduction of unstressed vowels), Croatian/Serbian has no
reduced vowels: every vowel is pronounced fully as spelled regardless of stress. The book's example:
`gòvorite` ("you speak") is pronounced with the same full vowel quality on every syllable, whereas a
Russian cognate-shaped word would reduce its unstressed vowels. Framed as a pronunciation-habit
warning for English/Russian-background learners who might otherwise slur unstressed syllables.
p. xii (preface).

### Accent placement and false friends (preface)

Accent in Cr&S does not necessarily fall on the final syllable (`sèstra`, `mikròskop` given as
accentuation examples). The preface also flags "false friends" with Russian as a source of possible
confusion/embarrassment for Russian-background learners: `urók` (Russian "lesson") vs. `ùrok` (Cr&S
"a spell/charm"), and `ponós` (Russian "diarrhea") vs. `pònos` (Cr&S "pride"). p. xii.

---

## Vision-reading confidence notes

This shard was produced by reading a scanned, image-only PDF directly via the Read tool's vision
capability — no OCR text layer was available. Honest self-assessment, as specifically requested for
this test dispatch:

**(a) Specific words/characters I was genuinely uncertain about:**

- **`gòspođa` vs. "gospoda" in the Razgovor dialogue (p. 1, line 13, both Croatian and Serbian
  columns).** The vocabulary-list headword on p. 2 is unambiguously `gòspođa` (with the đ diacritic
  clearly legible there), but the same word as it appears inline in the dialogue text sits close to
  the page's inner gutter, where a dark curved shadow from the book's binding partially obscures
  character detail. I could not fully confirm the đ diacritic on those two dialogue occurrences and
  am inferring the standard form from the clean vocabulary-list rendering rather than from the
  dialogue text itself.
- **Precise accent-mark subtype (grave vs. acute vs. circumflex-like marks).** Cr&S pedagogical
  texts commonly distinguish multiple pitch-accent types (short-falling, short-rising, long-falling,
  long-rising) using different diacritic shapes. I transcribed what I could visually distinguish as
  either a grave-like or acute-like mark per syllable, matching what appears in each word above, but
  at this scan resolution I cannot guarantee I've correctly distinguished every accent-mark subtype
  the source intended — the presence and rough position of stress/length marking is clear, but fine
  distinctions between accent categories carry lower confidence.
- **`viđénje` not being split into a Cr./S. pair.** Given the book's consistent ije/e jat pattern
  elsewhere, I expected a possible `vidjenje`/`viđenje` split, but the source presents only one form,
  `viđénje`, for both. I read this correctly as a single unsplit form rather than misreading a
  missing second column — noting it because it's a slight exception to the pattern I otherwise relied
  on to disambiguate faint diacritics elsewhere.
- **A handful of characters at the extreme left/right edges of lines on both facing pages** across
  all six page-images were affected by the same dark gutter shadow (visible as a broad dark vertical
  band down the center of every two-page spread) — mostly resolved successfully using context, but
  flagging the general risk.

**(b) Overall confidence: medium-high.** The scan is a clean typeset print (not handwritten, not
degraded/faded ink), with a legible serif body font and bold-faced Cr&S headwords that were easy to
distinguish from their English glosses in the two-column vocabulary layout. The main recurring
obstacle was the gutter shadow described above, not print quality itself. I'm confident in the
vocabulary items and their glosses overall; my lowest-confidence area is the fine-grained accent-mark
subtyping noted above, which I would not treat as reliable enough to build phonological analysis on
without a second pass or comparison against a clean digital source.

**(c) Scan-quality observations:**

- The scan reproduces two facing printed pages per image (a full book spread), with a pronounced
  dark curved shadow running down the spine/gutter of every spread — this is the single biggest
  recurring legibility obstacle, affecting the innermost portion of both the left and right page in
  each image.
- Page 4/5's spread (PDF page 9) contains what appear to be a previous owner's **handwritten pencil
  answers** filled into the blanks of "vježba/vežba IV" (e.g., what reads as "vé", "Dunja", "Kako" in
  the margin blanks). I identified these as reader marginalia, not source content, and correctly
  excluded them from extraction — flagging this because distinguishing print from handwritten
  annotation is exactly the kind of failure mode a less careful vision read could get wrong (e.g.
  misattributing a handwritten answer as if it were the printed textbook's own content).
- Diacritics (grave/acute accents, đ, č, ć, š, ž) were generally rendered with enough resolution to
  read confidently away from the gutter shadow; the exceptions are noted in (a) above.
- No pages in this assigned range showed genuine print degradation, ink bleed-through, or fading —
  the single recurring issue was the binding-shadow effect described above, which is a scanning
  artifact rather than a source-document quality problem.

---

## Copyright discipline reminder

The above is paraphrase + short illustrative quotes only — no bulk reproduction of the source's own
vocabulary boxes, dialogue blocks, or explanatory prose.
