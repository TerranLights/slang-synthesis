...
|---|---|---|---|
| Serbian/Croatian/Bosnian | not started *(test-run language)* | not started — `established/` has 32 Markdown files (Phase 1, full-book bridge-scale extraction complete: entire 531-page Alexander & Elias-Bursać flagship textbook — all 20 lessons, Appendices 1-6/9/7-8, full 5-part BCS-English glossary; ~338,000 words, ~8,452 vocabulary entries, subagent-parallelized across 28 chunks; own `graphify-out/` built (136 nodes, 206 edges, 10 communities); tentative, see `methodology-observations/serbian_croatian_bosnian_test_run.md`'s bridge-scale-test entry for the full-language-folder-scale readiness verdict); `analysis/`/`synthesized/` correctly empty pending `language_corpus/` progress | `datasets/Serbian_Croatian_Bosnian/` |
