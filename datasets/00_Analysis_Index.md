...
| Russian | in progress (Phase 3 corpus collection started: `Dermo!` slang dictionary) | **in progress — largest corpus in the project, not yet Phase 1 complete.** `established/` has 197 Markdown files (~1,161,500 words) from a 204-file/3.6GB source corpus (the largest and most heterogeneous triaged so far, ~75% scanned image-only). Waves 1-2 (31 clean-text sources) fully complete; Wave 3 (vision-reading) has 24 of ~45 prioritized sources done, including the slang flagship *Streetwise Russian*, the register-flagship *Using Russian: A Guide to Contemporary Usage*, and several sources yielding genuine criminal argot/youth slang/army slang finds; own `graphify-out/` not yet built (deferred until Wave 3 substantially lands, per this language's own checklist); two known non-blocking partial-coverage gaps (*Teach Yourself Russian* Units 14-20, *Everything in its Time* Section 12 onward) left open per honest partial-completion reporting; see `00_Extraction_Checklist.md` for the full per-book breakdown and `00_Book_Triage_Catalog.md` for the triage/priority record | `datasets/Russian/` |
